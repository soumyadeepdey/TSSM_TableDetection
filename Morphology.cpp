...
Mat IITkgp_functions::dilation(Mat image)
{
    int row = image.rows;
    int col = image.cols;
    int i,j;
    ...
    for(i=0;i<row;i++)
    {
        for(j=0;j<col;j++)
        {
            ...
            {	      
	      if(i-1>=0)
		tempimage.data[(i-1)*col+j]=0;
	      if(i+1<row)
		tempimage.data[(i+1)*col+j]=0;
	      if(j-1>=0)
		tempimage.data[i*col+(j-1)]=0;
	      if(j+1<col)
		tempimage.data[i*col+(j+1)]=0;
            }
        }
...
Mat IITkgp_functions::erosion(Mat image)
{
    int row = image.rows;
    int col = image.cols;
    int i,j;
...
Mat IITkgp_functions::boundaryextraction(Mat image)
{
 ...
 */
Mat IITkgp_functions::Erosion( int erosion_elem, int erosion_size, Mat image)
{
  int erosion_type;
 ...
 */
Mat IITkgp_functions::Dilation( int dilation_elem, int dilation_size, Mat image )
{
  Mat DilatedImage;
  int dilation_type;
 ...
 */
Mat IITkgp_functions::Open(int open_elem, int open_size, Mat image)
{
  Mat OpenImage;
 ...
 */
Mat IITkgp_functions::Close(int close_elem, int close_size, Mat image)
{
  Mat CloseImage;
  int close_type;
...
Mat IITkgp_functions::Morphology_Gui(Mat image, int type)
{
    Mat ProcessedImage;
